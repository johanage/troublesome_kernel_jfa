...
train_params = {
    ...
    "batch_size": 1,
    "loss_func": loss_func,
    "save_path": os.path.join(config.RESULTS_PATH_KADINGIR,"DIP", "detail_transfer", "large_detail"),
...
fn_init_weights = os.path.join(config.RESULTS_PATH_KADINGIR,"DIP", "detail_transfer","DIP_UNet_init_weights_%s_%.2f.pt"%(sp_type, sampling_rate) )
...
idx = 42
sample = torch.load(os.path.join(dir_train,"sample_%.5i.pt"%idx))
# sample is real valued so make fake imaginary part
sample = to_complex(sample[None, None]) + to_complex(large_det[None,None])
# simulate measurements by applying the Fourier transform
#measurement = OpA(sample + nullspace_data_det + to_complex(large_det[None,None])) # detailed image
measurement = OpA(sample) # only large detail included
...
z_tilde = torch.load(os.path.join(config.RESULTS_PATH_KADINGIR,"DIP", "detail_transfer", "z_tilde_%s_%.2f.pt"%(sp_type, sampling_rate)) )
...
save_log = False
if save_log:
    logging.to_pickle(os.path.join(config.RESULTS_PATH_KADINGIR, "DIP", "detail_transfer", "DIP_UNet_nojit_logging_{suffix}.pkl".format(suffix = fn_suffix) ))
else:
    logging = pd.read_pickle(os.path.join(config.RESULTS_PATH_KADINGIR, "DIP", "detail_transfer", "DIP_UNet_nojit_logging_{suffix}.pkl".format(suffix = fn_suffix) ))
...
for epoch in range(train_params["num_epochs"]):
    if epoch % train_params["save_epochs"] == 0:
            ...
            axs[1,isave].text(x = 5,y = 20, s = "PSNR : %.1f \nSSIM : %.2f"%(rec_psnr, rec_ssim), fontsize = 26, color="white")
            axs[0,isave].set_axis_off(); axs[1,isave].set_axis_off()
            isave += 1
        
fig.tight_layout()
#plotdir_dip = os.path.join(config.RESULTS_PATH_KADINGIR, "DIP", "detail_transfer", "plots")
plotdir_dip = os.path.join(config.PLOT_PATH, "DIP", "detail_transfer")
#fig_savepath = os.path.join(plotdir_dip)
#fig_savepath = os.path.join(plotdir_dip, "OG")
fig_savepath = os.path.join(plotdir_dip, "large_detail")
